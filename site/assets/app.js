...
function renderMarkdown(md){
  document.getElementById('viewer').innerHTML = '<pre>'+md
    .replace(/[&<>]/g, s=>({ '&':'&amp;','<':'&lt;','>':'&gt;' }[s]))+'</pre>';
}
async function search(query){
  const res = await fetch('index/route.json').then(r=>r.json());
  let shards = res.shards || ['shard_00.json'];
  const hits = [];
  for(const s of shards){
    const data = await fetch('index/'+s).then(r=>r.json());
    const searchFn = new Function('q', 'return ('+data.search+')(q)');
    for(const h of searchFn(query)){
      hits.push(h);
